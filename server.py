...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # K线直查接口: /api/kline?code=XXXXXX
        if parsed.path == '/api/kline':
            return self.handle_kline(parsed)

        # 实时行情接口: /api/quote?code=XXXXXX
        if parsed.path == '/api/quote':
            return self.handle_quote(parsed)
    ...
    def handle_kline(self, parsed):
        ...
        if not code or not code.isdigit() or len(code) != 6:
            self._json_error(400, '股票代码无效')
            return
        if code[0] in ('6', '9'):
            symbol = 'sh' + code
        else:
            symbol = 'sz' + code
        target = f'https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param={symbol},day,,,180,qfq'
        try:
            body, ct = self.fetch_remote(target)
            ...
            self._json_error(502, f'K线获取失败: {e}')
    ...
    def handle_quote(self, parsed):
        ...
        if code[0] in ('6', '9'):
            ...
            symbol = 'sz' + code
        target = f'https://qt.gtimg.cn/q={symbol}'
        try:
            body, ct = self.fetch_remote(target)
            self.send_response(200)
            self.send_header('Content-Type', ct)
            ...
            self._json_error(502, f'行情获取失败: {e}')

    def _json_error(self, code, msg):
        body = ('{"error":"' + msg.replace('"', '\\"') + '"}').encode('utf-8')
